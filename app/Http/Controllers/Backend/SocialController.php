    ...
    public function handleProviderCallback($provider)
    {
        ...
        if(Auth::check()){
            ...
            if(!Social::whereProviderAndSocial_id($provider, $social_user->id)->first()){
                $social->user_id = Backend::loggedInUser()->id;
                $social->save();
                $msg = trans('backend.social_success', ['provider' => $provider]);
            } else {
                $msg = trans('backend.social_error', ['provider' => $provider]);
            }

            if(Backend::loggedInUser()->isAdmin()){
                $url = route('backend::profile');
            }
            ...
            if($user = Backend::user('email', $social_user->email)){
                if(!Social::whereProviderAndSocial_id($provider, $social_user->id)->first()){
                ...
                $user = Backend::newUser();
                $user->email = $social_user->email;
                $user->name = $social_user->name;

                # Get the users settings
                $settings = Backend::userSettings();

                # Check if register is enabled
                if(!$settings->register_enabled) {
                    abort(403, trans('backend.error_registrations_disabled'));
                }
                ...
                if($settings->location){
                    try {
                        $user->country_code = Backend::getCountryCode($register_ip);
                    } catch (Exception $e) {
        ...
        if($logged_in){
            if($new_user){
                $msg = trans('backend.social_success_register', ['provider' => $provider]);
            } else {
                if($new_link){
                    $msg = trans('backend.social_success_login_link', ['provider' => $provider]);
                } else {
                    $msg = trans('backend.social_success_login', ['provider' => $provider]);
                }
            }
        }
