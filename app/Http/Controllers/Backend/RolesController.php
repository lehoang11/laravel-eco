...
class RolesController extends Controller
{
    ...
    {
        Backend::permissionToAccess('backend.roles.access');
    	...
    	# Return the view
    	return view('backend/roles/index', ['roles' => $roles, 'default_role' => $default_role]);
    }
    ...
    public function edit($id)
    {
        ...
        # Check permissions
        Backend::permissionToAccess('backend.roles.edit');

        # Find the role
        $row = Role::findOrFail($id);

        if(!$row->allow_editing and !Backend::loggedInuser()->su) {
            abort(403, trans('backend.error_editing_disabled'));
        }
    ...
    public function update($id, Request $request)
    {
        ...
        return redirect()->route('backend::roles')->with('success', trans('backend.msg_role_edited'));
    }
    ...
    public function create()
    {
        ...
        # Check permissions
        Backend::permissionToAccess('backend.roles.create');
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('backend::roles')->with('success', trans('backend.msg_role_created'));
    }
    ...
    public function editPermissions($id)
    {
        ...
        # Check permissions
        Backend::permissionToAccess('backend.roles.permissions');

    	# Find the role
    	$role = Backend::role('id', $id);

        if(!$role->allow_editing and !Backend::loggedInuser()->su) {
            abort(403, trans('backend.error_editing_disabled'));
        }
    ...
    public function setPermissions($id, Request $request)
    {
    	...
    	foreach($permissions as $perm){
            ...
            if(!$perm->assignable and !Backend::loggedInUser()->su) {
                $modify = false;
            }
    	...
    	# Return a redirect
    	return redirect()->route('backend::roles')->with('success', trans('backend.msg_role_perms_updated'));
    }
    ...
    public function destroy($id)
    {
        ...
        # Check permissions
        Backend::permissionToAccess('backend.roles.delete');
        ...
        # Check if it's su
        if($role->su) {
            return abort(403, trans('backend.error_security_reasons'));
        }

        # Check if it's the default role
        if($role->id == Backend::defaultRole()->id) {
            abort(403, trans('backend.error_security_reasons_default_role'));
        }
    	...
    	# Redirect the admin
    	return redirect()->route('backend::roles')->with('success', trans('backend.msg_role_deleted'));
    }
}
